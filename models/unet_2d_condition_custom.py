...
class UNet2DConditionCustomModel(
    ...
    def __init__(
        ...
        if extra_cond_channels > 0:
            self.extra_cond = ConditioningEmbedding(
                conditioning_embedding_channels=extra_in_channels,
                conditioning_channels=extra_cond_channels,
            )
    ...
    def forward(
        ...
        if self.extra_cond is not None:
            extra_sample = self.extra_cond(extra_condition)
            sample = torch.cat([sample, extra_sample], dim=1)
